...
app_ui = ui.page_fluid(
    ui.row(
        ui.panel_well("Where can I get a bike? -- Capital Bikeshare Python (DEVELOPMENT VERSION)"),
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def plot():
        if station():
            ...
            print(f"Posting prediction to `{endpoint}`...")
            df_to_plot_id["pred"] = predict(endpoint, df_to_pred)
            print(f"Response received from `{endpoint}`")
